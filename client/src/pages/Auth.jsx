...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-white dark:bg-zinc-900 transition-colors">
      <div className="absolute top-4 right-4">
        <Button className="rounded-full" onClick={() => dispatch(toggleTheme())}>
          {theme === "light" ? "Dark" : "Light"}
        </Button>
      </div>
      <div className="w-full max-w-5xl bg-white dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 shadow-xl rounded-2xl flex flex-col xl:flex-row overflow-hidden">
        <div className="flex-1 p-6 md:p-10 flex flex-col gap-6 items-center justify-center">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-white">Welcome</h1>
            <img className="mx-auto mt-2 h-14" src="http://www.i2symbol.com/pictures/emojis/2/5/d/7/25d7afe627a6de5c89acfc91c7f1f3bf.png" alt="Victory emoji" />
            <p className="mt-2 text-gray-600 dark:text-gray-300 text-sm">Fill in the details to experience the most amazing chat app!</p>
          </div>

          <Tabs className="w-full" defaultValue="login">
            <TabsList className="flex w-full bg-gray-100 dark:bg-zinc-700 rounded-md overflow-hidden">
              <TabsTrigger className="w-full px-4 py-2 text-sm font-medium data-[state=active]:bg-purple-100 data-[state=active]:dark:bg-purple-900 data-[state=active]:text-purple-700 dark:data-[state=active]:text-white transition" value="login" onClick={() => setErrorMessage(null)}>
                Login
              </TabsTrigger>
              <TabsTrigger className="w-full px-4 py-2 text-sm font-medium data-[state=active]:bg-purple-100 data-[state=active]:dark:bg-purple-900 data-[state=active]:text-purple-700 dark:data-[state=active]:text-white transition" value="signup" onClick={() => setErrorMessage(null)}>
                Signup
              </TabsTrigger>
            </TabsList>

            <TabsContent value="login" className="mt-6 flex flex-col gap-4">
              <Input placeholder="Email" type="email" className="rounded-md p-3" value={email} onChange={(e) => setEmail(e.target.value)} />
              <Input placeholder="Password" type="password" className="rounded-md p-3" value={password} onChange={(e) => setPassword(e.target.value)} />
              <Button disabled={loading} className="rounded-md p-3 dark:bg-zinc-700 dark:text-white" onClick={handleLogin}>
                {loading ? (<><Loader2 className="h-5 w-5 animate-spin mr-2" /><span>Loading ...</span></>) : "Login"}
              </Button>
              <OAuth />
            </TabsContent>

            <TabsContent value="signup" className="mt-6 flex flex-col gap-4">
              <Input placeholder="Email" type="email" className="rounded-md p-3" value={email} onChange={(e) => setEmail(e.target.value)} />
              <Input placeholder="Password" type="password" className="rounded-md p-3" value={password} onChange={(e) => setPassword(e.target.value)} />
              <Input placeholder="Confirm Password" type="password" className="rounded-md p-3" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} />
              <Button disabled={loading} className="rounded-md p-3 dark:bg-zinc-700 dark:text-white" onClick={handleSignUp}>
                {loading ? (<><Loader2 className="h-5 w-5 animate-spin mr-2" /><span>Loading ...</span></>) : "Signup"}
              </Button>
              <OAuth />
            </TabsContent>
          </Tabs>
          ...
          {otp && (
            <div className="w-full mt-4 flex flex-col gap-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Enter Your 6-digit OTP:</label>
              <input onChange={(e) => setOtpValue(e.target.value)} className="p-2 rounded-md border dark:border-white" type="text" />
              <div className="flex gap-2">
                <button onClick={handleOtpSubmit} className="flex-1 bg-green-500 text-white py-2 rounded-md">Enter</button>
                <button onClick={handleResendOtp} className="flex-1 bg-blue-500 text-white py-2 rounded-md">Resend OTP</button>
              </div>
            </div>
          )}

          {errorMessage && <div className="text-sm text-red-500 truncate max-w-sm">{errorMessage}</div>}
          {successMessage && <div className="text-sm text-green-600 truncate max-w-sm">{successMessage}</div>}
          {message && <div className="text-sm text-blue-600 truncate max-w-sm">{message}</div>}
        </div>

        <div className="hidden xl:block xl:w-1/2">
          <img src="https://wallpaperaccess.com/full/3404308.jpg" alt="background" className="w-full h-full object-cover" />
        </div>
      </div>
    </div>
  );
